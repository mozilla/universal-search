...
function RecommendationServer(opts) {
  ...
  */
  ...
  this.resultClasses = opts.resultClasses;
...
RecommendationServer.prototype = {
  ...
  getResultClass: function(data) {
    let types = Object.keys(data.enhancements);
    if (types.indexOf('tld') > -1) {
      return this.resultClasses.tld;
    } else if (types.indexOf('wikipedia') > -1) {
      return this.resultClasses.wikipedia
    }
    return null;
  },
  ...
  search: function(payload) {
    ...
    server._search(payload.query).then(data => {
      let resultClass = server.getResultClass(data);
      if (resultClass) {
        server.console.log(resultClass.name, data);
        server.events.publish('recommendation', {
          payload: data,
