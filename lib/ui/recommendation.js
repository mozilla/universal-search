...
Cu.import('chrome://universalsearch-results/content/result-tld.js');
Cu.import('chrome://universalsearch-results/content/result-wikipedia.js');
...
Recommendation.prototype = {
  ...
  // Passed a response from the recommendation server, negotiates for and
  // returns an appropriate result class.
  getResultClass: function(data) {
    if ('tld' in data.enhancements) {
      return TLDResult;
    } else if ('wikipedia' in data.enhancements) {
  ...
  show: function(data) {
    ...
    // If the recommendation wrapper hasn't been inserted into the XUL DOM yet,
    // there's no need to 
    if (!this.el) {
      return;
    }
    ...
    if (this.data && this.el.firstChild &&
        data.result.url === this.data.result.url) {
      this.el.collapsed = false;
      this.events.publish('recommendation-shown', this);
      return;
    }

    const ResultClass = this.getResultClass(data);
    if (ResultClass) {
      const recommendation = this.row = new ResultClass(data, {
        eTLD: this.eTLD,
        io: this.io,
        win: this.win
      }).render();
