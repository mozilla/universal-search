...
Recommendation.prototype = {
  ...
  show: function(data) {
    if (this.hideTimeout) {
      this.win.console.log('show clearing hideTimeout');
  ...
  hide: function(isImmediate) {
    ...
    // this.hide is called by some pubsub signals, which pass in truthy
    // arguments, but we only want to immediately hide if the argument was
    // the boolean 'true'.
    if (isImmediate === true) {
      ...
      this.hideTimeout = this.win.setTimeout(this._hide, 100);
    }
  },
  _hide: function() {
  ...
  duplicateCheck: function() {
    ...
    for (let i = 0; i < this.searchResults.length; i++) {
      ...
      if (data.type === 'tld' && this.eTLD.getBaseDomain(resultUri) === this.eTLD.getBaseDomain(recommendationUri) ||
          data.type === 'wikipedia' && resultUri.equals(recommendationUri)) {
        return;
      }
    }
