...
RecommendationRow.prototype = {
  ...
  renderContent: function(row) {
    ...
    - the url
    */
    // TODO: remove the content ID styles from the CSS, replace with these.
    ...
    if (row._data.details || row._data.description) {
      let extras = createElement(row.doc, 'hbox', 'extras');
      if (row._data.details) {
        extras.appendChild(row.renderDetails(row));
      }
      if (row._data.description) {
        elem.appendChild(row.renderDescription(row));
      }
      elem.appendChild(extras);
    }
