...
RecommendationRow.prototype = {
  ...
  renderKeyImage: function(row, parentElem, keyImage) {
    ...
    */
    ...
    // is assumed to exist. This is correct, because `elem` is returned before
    // the promise resolves, and the caller appends `elem` to the DOM in the
    // same turn. This is one case where it's really nice that promise
    // resolution requires waiting a turn :-)
    row.win.fetch(keyImage.url, {
      ...
      response.blob().then(imgBlob => {
        ...
        elem.parentNode.setAttribute('height', keyImage.height);
        elem.parentNode.setAttribute('width', keyImage.width);
      });
    }, (err) => {
      elem.parentNode.replaceChild(row.renderLetterbox(row), elem);
      elem.parentNode.removeAttribute('height', DEFAULT_IMAGE_SIZE);
      elem.parentNode.removeAttribute('width', DEFAULT_IMAGE_SIZE);
    });
  ...
  renderTitle: function(row) {
    /*
    Creates and returns a <description> element containing the
